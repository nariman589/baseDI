...
import { jDescriptionsProps } from "types/branchTypes";
import {
	AccordeonList,
	AccordeonListTitle,
	AccordeonListName,
...
interface Props {
	data: jDescriptionsProps;
	enableEdit?: boolean;
}

function JDescriptionCard({ data, enableEdit = false }: Props) {
		...
		<AccordeonList>
			<AccordeonListTitle isActive={isActive || editStatus}>
				...
				<AccordeonItems>
					...
					<AccordeonButtonWrapper>
						<AccordeonButton
							src={isActive ? CloseViewIcon : ViewIcon}
							onClick={() => {
								setActive((v) => !v);
								setEditStatus(false);
							}}
...
export default JDescriptionCard;
